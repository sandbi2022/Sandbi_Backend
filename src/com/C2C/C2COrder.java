...
public class C2COrder {
	...
	public static void createOrder(String tradePair, String User, int tradeType, double amount, double maxAmount,
			double minAmount, double price) {
	...
	public static void cancelPendingOrder(String TID, String tradePair) {
		...
		try {
			...
			if (pendingTrade.getTradeType() == 1) {
				unfreeze(pendingTrade.getSeller(), tradePair,
						(pendingTrade.getAmount() - pendingTrade.getDoneAmount()));
			}
	...
	public static C2CTrade findPendingTrade(String TID, String tradePair) {
		...
		C2CTrade pendingTrade = new C2CTrade(tradePair);
		pendingTrade.setTradeState(-1);
		String sql = "Select * from C2CPendingTrade." + tradePair + " where TID=\"" + TID
	...
	public static void creatPendingC2CTrade(C2CTrade pendingOrder) {
		String sql = "insert into C2CPendingTrade." + pendingOrder.getTradePair() + " value(\"" + pendingOrder.getTid()
				+ "\",\"" + pendingOrder.getUser() + "\"," + pendingOrder.getAmount() + ","
				+ pendingOrder.getMinAmount() + "," + pendingOrder.getMaxAmount() + "," + pendingOrder.getDoneAmount()
				+ "," + pendingOrder.getPrice() + "," + pendingOrder.getTradeType() + "," + pendingOrder.getTradeState()
				+ ")";
		try {
			Class.forName(JDBC_DRIVER);
			Connection con = DriverManager.getConnection(DB_URL, USER, PASS);
	...
	public static void creatHistoryC2CTrade(C2CTrade C2CTrade) {
		...
		String sql = "insert into C2CTrade." + C2CTrade.getTradePair() + " value(\"" + C2CTrade.getTid() + "\",\""
				+ C2CTrade.getSeller() + "\",\"" + C2CTrade.getBuyer() + "\"," + C2CTrade.getAmount() + ","
				+ C2CTrade.getPrice() + ",\"" + time + "\",0);";
		try {
			Class.forName(JDBC_DRIVER);
			Connection con = DriverManager.getConnection(DB_URL, USER, PASS);
			Statement st = con.createStatement();
	...
	public static void changePendingTrade(C2CTrade pendingTrade) {
		String sql = "update C2CPendingTrade." + pendingTrade.getTradePair() + " set doneAmount = "
				+ pendingTrade.getDoneAmount() + ", State = " + pendingTrade.getTradeState() + " where TID=\""
				+ pendingTrade.getTid() + "\";";
		try {
			Class.forName(JDBC_DRIVER);
			Connection con = DriverManager.getConnection(DB_URL, USER, PASS);
